...
export class QrcodeDashboardComponent implements OnInit {
    ...
    onChangeContent (): void {
        ...
        if([4, 38, 47].indexOf(this.bankCode) > -1) {
            console.log('1');
            if(this.bankCode === 4) {
                this.accountNo = `96296${this.accountNumber}${this.subNumber}`;
            } else {
                this.accountNo = `068C${this.accountNumber}${this.subNumber}`;
            }
            ...
            this.content = `Nop tien vao tk ck 068C${this.accountNumber}${this.subNumber} ${this.accountName}`;
        }
        // 38 techcombank
        // 4 BIDV
        // 47 VPB
